...
/*  09-Nov-17	Christopher G. Healey	Updated retrive_page_v_1_1 to take  */
/*					extended tweet JSON, get full body  */
/*					whether original tweet or retweet   */
/*--------------------------------------------------------------------------*/
...
var  graph_nodes = new Array();
var  graph_links = new Array();
...
// ################################################################
// Jianbo - Sept 28th, 2018
// created based on query_twitter_v_1_1() and retrieve_page_v_1_1()
// ################################################################
//function disco_main( term, pg_tot )
function disco_main()
  ...
  //  pg_tot:  Pages to request (max 10)
{
  ...
  */
  ...
  // Set 30-second timeout on HTTP get
  ...
  query_dlg_title( "Working" );
  query_dlg_msg( "Analyzing data and generating visualization..." );
  ...
  disco_main_process();    // Queue pages
} 

function disco_main_process()
{
  ...
  //alert("term:"+term+" url:"+url);
  //term: trump url: https://www.csc2.ncsu.edu/faculty/healey/tweet_viz/tweet_app/php/recent.php?callback=?&q=trump&pg=1
  ...
  $.ajax({
    ...
    success: function(data){
      ...
      var  body;      // Body of tweet
      var  i;       // Loop counters
      //var  lim_s;     // Rate limit debug information
      var  pos;       // Position of end of retween
      var  tw_obj;      // JSON tweet object

      //$('#tweet_response').text(data);

      if ( typeof data == "undefined" ) {
        alert("getJSON returns undefined data");
        console.log( "getJSON returns undefined data" );
        console.log( data );
      } else {        // Data returned successfully
        if (data != undefined){
            sentiment_json = JSON.parse(data[0]);       //json response for sentiment analysis
            affinity_node_json = JSON.parse(data[1])    //json response for affinity analysis - nodes
            affinity_link_json = JSON.parse(data[2])    //json response for affinity analysis - links

            // parse sentiment response
            if(sentiment_json.length>0){
              for( i = 0; i < sentiment_json.length; i++ ) {
                tip = sentiment_json[i].tip;
                tip = tip.replace(/\[NV\]/g,"<font color='#C0392B'><i><b>");
                tip = tip.replace(/\[PV\]/g,"<font color='#28B463'><i><b>");
                tip = tip.replace(/\[E\]/g,"</b></i></font>");
                tip = tip.replace(/\\n/g,"<br>");
                tip = tip.replace(/\\/g,"");
                sentiment_json[i].tip = tip;
                sentiment_json[i].fmt = tip;
                tw.push( sentiment_json[i] );
              }
            }
            // parse affinity response
            graph_nodes = affinity_node_json;
            graph_links = affinity_link_json;
        }
        /*
        for( i = 0; i < data.tw.length; i++ ) {
          ...
          //  To get full_text of an original tweet, look for full_text
          //  field, then text field; to get full_text of a retween, look
          //  for full_text then text in the retweeted_status dictionary
          //  Grab initial tweet body, assuming it's not a retweet

          if ( data.tw[ i ].full_text != undefined ) {
            body = data.tw[ i ].full_text;
          } else if ( data.tw[ i ].text != undefined ) {
            body = data.tw[ i ].text;
            console.log( "WTF? Found a text field..." )
          } else {
            continue;
          }

          //  If retweet, grab RT @ text, append original body

          if ( data.tw[ i ].retweeted_status != undefined ) {
            pos = body.indexOf( ":" );
            body = body.substring( 0, pos ) + ": ";
            body = body + data.tw[ i ].retweeted_status.full_text;
          }
      ...
      $("#prog-bar" ).progressbar( "option", "value", 50 );

      //setTimeout( "update_load()", 100 );
      update_load();

      $("#prog-bar" ).progressbar( "option", "value", 100 );
  ...
  //  Parse text on successful query 
	...
	//  forcing an update_load() to process any data received
        ...
        pg = pg_req - block_sz;
	      ...
	      lim_s = data.limit + "/" + data.remain;
        lim_s = lim_s + " (" + new Date( data.reset * 1000 ) + ")";
        console.log( lim_s );
        max_id = data.max_id;
